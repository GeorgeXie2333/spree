...
A CenWatch customer storefront for `shop.cenwatch.com`, built on Next.js 16, React 19, and the [Spree REST API](https://spreecommerce.org/docs/api-reference). The storefront is designed for Docker/Node self-hosting with Spree + Stripe as the primary commerce path.

## CenWatch Self-Hosted Deployment

Use `shop.cenwatch.com` for the Next.js storefront and keep the Spree API on a separate HTTPS origin such as `api.cenwatch.com` or a hosted Spree domain. The public `cenwatch.com` site can remain the content/reference domain until a later redirect decision.

Required storefront env:

```env
SITE_URL=https://shop.cenwatch.com
NEXT_PUBLIC_SITE_URL=https://shop.cenwatch.com
SPREE_API_URL=https://api.cenwatch.com
...
NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_or_live_stripe_publishable_key
```

Stripe is the first supported payment path. Keep Stripe secret keys and payment method setup in Spree/backend configuration; the storefront only needs the publishable key required by Stripe client components.

Order tracking is exposed at `POST /api/v3/store/order_tracking`. Configure `ORDER_TRACKING_API_URL` to a private backend endpoint that performs the exact order-number/email match and returns only limited status/tracking fields; `ORDER_TRACKING_API_SECRET` is forwarded as `x-order-tracking-secret` when set.

Backend/admin checklist:

- Create a Spree publishable API key scoped to the CenWatch shop/store/channel.
- Add Allowed Origin entries for `https://shop.cenwatch.com` and local development origins such as `http://localhost:3001`.
- Configure the Spree store URL/storefront URL to `https://shop.cenwatch.com`.
- Configure Stripe in Spree and verify a full test payment from cart to order confirmation.
...
docker build -f apps/storefront/Dockerfile \
  ...
  --build-arg SPREE_API_URL=https://api.cenwatch.com \
  --build-arg SPREE_PUBLISHABLE_KEY=pk_live_or_test_publishable_key \
  --build-arg SPREE_CHANNEL_CODE=shop \
  --build-arg SPREE_VALIDATE_MARKETS=false \
  --build-arg NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_or_live_stripe_publishable_key \
...
- Terminate TLS for `shop.cenwatch.com`.
- Proxy traffic to the container on `127.0.0.1:3001`.
- Preserve `Host`, `X-Forwarded-Host`, `X-Forwarded-Proto`, and `X-Forwarded-For`.
- Do not proxy `/api/v3/*` through the storefront; browser and server traffic should call `SPREE_API_URL` on the API domain.
...
[TypeScript SDK](https://www.npmjs.com/package/@spree/sdk) | [Spree Store API](https://spreecommerce.org/docs/api-reference)
...
**Multi-region out of the box.** Country, currency, and language switching via URL segments (`/us/en/`, `/us/zh/`, `/ca/en/`, `/ca/zh/`), powered by Spree [Markets](https://spreecommerce.org/docs/developer/core-concepts/markets). Distinct selling regions bundling geography, currency, and locale in a single store.
...
pnpm --filter @cenwatch/storefront start
```
...
```bash
pnpm --filter @cenwatch/storefront test            # one-shot
pnpm --filter @cenwatch/storefront test:watch      # watch mode
```
...
pnpm --filter @cenwatch/storefront test:e2e:ui
...
/us/zh/products          # US Market, Simplified Chinese
/ca/en/products          # Canada Market, English
/ca/zh/products          # Canada Market, Simplified Chinese
```
...
Run the storefront in development (`pnpm --filter @cenwatch/storefront dev`) and visit [http://localhost:3001/dev/emails](http://localhost:3001/dev/emails). Each template is rendered with sample data using `@react-email/render`; the route is gated to non-production environments.
...
A multi-stage `Dockerfile` is included for the CenWatch storefront. It uses Next.js standalone output on `node:22-alpine`, runs as a non-root user, and exposes port `3001`.

> **Note:** `SITE_URL` is required at build time for metadata and sitemap URLs. `SPREE_API_URL` and `SPREE_PUBLISHABLE_KEY` are required at runtime for products, cart, account, and checkout. The build uses CenWatch US/Canada launch market fallback when the API is unavailable; set `SPREE_VALIDATE_MARKETS=true` only when the storefront should validate URL countries against live Spree Markets.
