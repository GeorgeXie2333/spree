...
```bash
# Lint the codebase
pnpm --filter @cenwatch/storefront lint

# Format all files
pnpm --filter @cenwatch/storefront format

# Run both lint and format checks
pnpm --filter @cenwatch/storefront check
```

Always use `pnpm --filter @cenwatch/storefront check` before committing changes and fix any issues with `pnpm --filter @cenwatch/storefront format`.
